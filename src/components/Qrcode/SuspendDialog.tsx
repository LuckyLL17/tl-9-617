...
type StepType = 'action' | 'reason' | 'confirm';
...
export default function SuspendDialog({
  ...
  const getTitle = () => {
    if (step === 'action') return '二维码管理';
    if (step === 'reason') {
      return selectedAction === 'suspend' ? '暂停使用' : '挂失冻结';
    }
    return '确认操作';
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      <div
        ...
        onClick={handleClose}
      />
      <div className="relative w-full sm:max-w-md bg-white sm:rounded-2xl rounded-t-3xl flex flex-col overflow-hidden" style={{ maxHeight: '85vh' }}>
        <div className="flex-shrink-0 px-4 sm:px-6 py-4 border-b border-gray-100 flex items-center justify-between z-10">
        ...
        {step === 'reason' && (
          <div className="space-y-4">
            <p className="text-gray-500 text-sm">
              {selectedAction === 'suspend' ? '选择暂停原因' : '选择挂失原因'}
            </p>
            <div className="space-y-3">
              {(selectedAction === 'suspend' ? suspendReasons : lostReasons).map((reason) => (
                <button
                  key={reason.id}
            ...
            <div className="flex flex-col items-center text-center">
              <div
                className={cn(
                  ...
                  getConfirmInfo().iconColor
                )}
              >
                <AlertTriangle size={32} className={getConfirmInfo().iconTextColor} />
              </div>
              <h3 className="text-lg font-bold text-gray-900 mb-2">
                {getConfirmInfo().title}
              </h3>
              <p className="text-sm text-gray-500">{getConfirmInfo().desc}</p>
            </div>
        ...
        {showFooter && (
          <div className="flex-shrink-0 px-4 sm:px-6 py-4 border-t border-gray-100 bg-white z-10 relative">
            {step === 'reason' && (
              <button
                onClick={handleBack}
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    getConfirmInfo().buttonColor
                  )}
                >
                  {getConfirmInfo().buttonText}
                </button>
              </div>
            )}
          </div>
